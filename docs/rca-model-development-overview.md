...
프로젝트의 최종 목표는 단순히 바닐라 모델보다 나아지는 것이 아니다. **동일한 RCA 하네스와 동일한 봉인 시나리오에서 Student가 Claude·Codex와 비슷하거나 더 높은 성능을 내는 것**이다. `RL ≥ SFT ≥ vanilla`는 최종 목표로 가기 위한 내부 비퇴행 조건이고, 최종 성공 조건은 `Student ≥ Claude/Codex teacher benchmark`다.
...
프로젝트의 공통 골격은 다음과 같이 고정한다.

> 교사 조사 데이터 생성 → 전체 episode SFT → SFT 모델 위의 RL → 공정 평가

즉, 실험마다 하네스나 학습법은 달라질 수 있어도 **바닐라에서 곧바로 RL을 돌리는 것이 아니라, 먼저 SFT로 검증된 조사 습관을 만든 뒤 그 SFT 정책을 시작점으로 RL을 수행한다.** 한 실험 주기는 다음처럼 운영한다.

1. 하네스·교사 데이터·base model·학습 파라미터 조합을 하나 정한다.
2. 전체 조사 episode로 SFT adapter를 학습한다.
3. 그 SFT adapter를 기준 정책으로 삼아 RL adapter를 학습한다.
4. train monitor와 development holdout에서 `RL ≥ SFT ≥ vanilla`인지 확인한다.
5. 통과하지 못하면 봉인셋을 열지 않고 원인을 분석해 다음 조합으로 다시 1~4를 반복한다.
6. 내부 조건을 통과한 후보만 설정을 동결하고, 마지막에 Claude·Codex와 동일한 봉인 평가를 수행한다.

그 안에서는 다음 변수를 계속 바꾸며 반복 실험한다.

- Student 하네스의 도구, 관측 표현, 종료 정책, proof 계약
- Claude·Codex 교사 조합과 실패 비평·교정 방식
- 학습 시나리오 구성과 성공/실패 궤적 선별 방식
- base model과 LoRA rank·alpha·학습률·epoch
- SFT loss weighting과 episode 구성
- RL 알고리즘, preference pair, KL/anchor 강도와 학습률
- 추론 token 한도, sampling 설정과 동적 종료 기준

다만 최종 봉인셋 결과를 보고 위 변수를 반복 조정하면 그 봉인셋은 사실상 개발셋이 된다. 따라서 반복 튜닝은 train 내부 monitor 또는 별도 development holdout에서 수행하고, Claude·Codex와의 최종 비교용 봉인셋은 후보와 설정을 모두 고정한 뒤 제한적으로 실행해야 한다.
...
이 방식은 SFT가 만든 좋은 기본 정책을 보존하면서, 실패 궤적과의 경계를 더 선명하게 만들려는 것이다. 현재 20개 시나리오에서 만든 20개 전체 궤적 쌍으로 20-step v6 학습을 진행 중이다.
...
| SFT 공정 모니터 평가 | 완료 | 6 case × 3회 = 18/18. 형식 오류 0, 근거 없는 confirmed 0, evidence complete 18/18. strict 2/18, 평균 root F1 0.222로 원인 식별력 개선 필요 |
| 초기 RL v1~v5 | 실험 완료 | 일부 실험에서 SFT 대비 퇴행 확인. 최종 후보로 확정하지 않음 |
| 개선 RL v6 | 진행 중 | 20개 전체 궤적 쌍을 사용하는 20-step RPO 방식 |
...
2. 완료된 SFT 모니터를 기준선으로 고정하고 낮은 원인 식별력과 metric action 미사용 원인을 분석한다.
...
7. 내부 비퇴행 후보를 고정한 뒤 Claude·Codex를 같은 봉인 계약으로 평가해 최종 parity를 비교한다.
8. 보류한 교사 case 3건은 증거 결함과 추론 실패를 구분해 별도 완성한다.
...
- 동일한 하네스·도구·case·반복 횟수·scorer 조건에서 Student가 Claude와 Codex의 봉인 성능에 도달하거나 앞선다.
- teacher 비교 전까지 최종 봉인 case별 결과를 학습 데이터, 교정 힌트, prompt 또는 하이퍼파라미터 선택에 사용하지 않는다.
