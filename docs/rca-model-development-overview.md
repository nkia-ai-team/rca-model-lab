...
평가 보고서에는 조사 효율과 도구 성공률도 기록하지만 RL 보상에서는 제외한다. 원인이 틀린 두 실행을 단순히 턴 수나 호출 성공률로 서열화하면, 빨리 틀리거나 의미 없는 유효 호출을 하는 정책을 강화할 수 있기 때문이다. 오프라인 RL 데이터와 Prime-RL 온라인 환경은 위의 동일한 진단 보상 함수를 사용한다.
